...
// import { IGroupMember } from '../services/sqlite.service';
...
//   get groupMembersArray() {
//   return this.groupMembers ? Object.keys(this.groupMembers).map(k => ({ id: k, ...this.groupMembers[k] })) : [];
// }
...
//   const buttons: ActionSheetButton[] = [
//     { text: t.instant('userabout.actions.message'), icon: 'chatbox', handler: () => this.messageMember(member) }
//   ];

//   const memberId = String(member?.user_id ?? member?.userId ?? member?.id ?? '');
//   const groupId = this.receiverId || this.groupId;

//   // get admin info (always hits RTDB)
//   const isCurrentUserAdmin = await this.firebaseChatService.isUserAdmin(groupId, this.currentUserId);
//   const isTargetUserAdmin = await this.firebaseChatService.isUserAdmin(groupId, memberId);
//   const isSelf = memberId === String(this.currentUserId);
...
//     delete this.groupMembers[member.user_id];

//     const toast = await this.toastCtrl.create({
//       message: this.translate.instant('userabout.toasts.removedFromGroup', { name: member.username || member.name }),
//       duration: 2000,
//       color: 'success'
//     });
//     await toast.present();
